...
from torchvision.datasets import PhotoTour, Kinetics, HMDB51, UCF101, Cityscapes, CocoCaptions, CocoDetection, \
...
def get_torchvision_dataset(dataset_cls, dataset_params_config):
    ...
    if dataset_cls in (PhotoTour, Kinetics, HMDB51, UCF101):
...
def split_dataset_equally(org_dataset, split_config, dataset_id, dataset_dict):
    org_dataset_length = len(org_dataset)
    logger.info('Splitting `{}` dataset ({} samples in total)'.format(dataset_id, org_dataset_length))
    lengths = split_config['lengths']
    train_lengths = int(500 * lengths[0])
    val_lengths = int(500 * lengths[1])
    manual_seed = split_config.get('generator_seed', None)

    train_indices = []
    val_indices = []
    labels = np.array(org_dataset.targets)

    for i in range(100):
        indices = np.where(labels == i)[0]
        # Split the indices into train and validation sets
        splitter = StratifiedShuffleSplit(n_splits=1, test_size=lengths[1], train_size=lengths[0], random_state=manual_seed)
        y = np.zeros(len(indices))
        train_index, val_index = next(splitter.split(indices, y))

        # temp1, temp2, temp3, temp4 = train_index[:125], train_index[125:], val_index[:125], val_index[125:]
        # train_index = np.concatenate((temp1, temp3))
        # val_index = np.concatenate((temp2, temp4))

        # # Add the indices to the appropriate lists
        train_indices.extend(indices[train_index][:train_lengths])
        val_indices.extend(indices[val_index][:val_lengths])

    train_set = Subset(org_dataset, train_indices)
    val_set = Subset(org_dataset, val_indices)
    sub_datasets = [train_set, val_set]
    for sub_dataset in sub_datasets:
        sub_dataset.dataset = copy.deepcopy(sub_dataset.dataset)

    sub_splits_config = split_config['sub_splits']
    ...
    if dataset_type == 'cocodetect':
        ...
        for split_name in dataset_splits_config.keys():
            ...
            if random_split_config is None:
                ...
                split_dataset_equally(org_dataset, random_split_config, dataset_id, dataset_dict)
