...
def compute_agreement(teacher_outputs, student_outputs, targets):
    with torch.no_grad():
        ...
        top1_agree = teacher_preds.eq(student_preds).sum.item()
...
@torch.inference_mode()
def get_agreement(teacher_model, student_model, data_loader, device, log_freq=1000, header='Test:', train_agreement=False):
