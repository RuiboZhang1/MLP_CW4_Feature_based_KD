...
def compute_agreement(teacher_outputs, student_outputs, targets):
    with torch.no_grad():
        batch_size = targets.size(0)
        teacher_preds = teacher_outputs.argmax(dim=1, keepdim=True)
        student_preds = student_outputs.argmax(dim=1, keepdim=True)
        teacher_preds = teacher_preds.t()
        student_preds = student_preds.t()
        agrees = teacher_preds.eq(student_preds)
        top1_agree = agrees[:1].flatten().sum(dtype=torch.float32)
        return top1_agree * (100.0/batch_size)
...
@torch.inference_mode()
def get_agreement(teacher_model, student_model, data_loader, device, log_freq=1000, header='Agreement: ', train_agreement=False):
    ...
    if train_agreement:
        for image, target, _ in metric_logger.log_every(data_loader, log_freq, header):
            image = image.to(device, non_blocking=True)
            target = target.to(device, non_blocking=True)
            teacher_output = teacher_model(image)
            student_output = student_model(image)
            top1_agree = compute_agreement(teacher_output, student_output, target)
            batch_size = image.shape[0]
            metric_logger.meters['agree1'].update(top1_agree.item(), n=batch_size)

        metric_logger.synchronize_between_processes()
        top1_agreement = metric_logger.agree1.global_avg
        logger.info(' *Train Agreement@Top1 {:.4f}\n'.format(top1_agreement))        
    else:
        for image, target in metric_logger.log_every(data_loader, log_freq, header):
            image = image.to(device, non_blocking=True)
            target = target.to(device, non_blocking=True)
        ...
        metric_logger.synchronize_between_processes()
        top1_agreement = metric_logger.agree1.global_avg
        logger.info(' *Test Agreement@Top1 {:.4f}\n'.format(top1_agreement))
    return metric_logger.agree1.global_avg
...
def train(teacher_model, student_model, dataset_dict, ckpt_file_path, device, device_ids, distributed, config, args):
    ...
    for epoch in range(args.start_epoch, training_box.num_epochs):
        ...
        # if epoch == training_box.num_epochs-1:
        #     train_top1_agreement = get_agreement(teacher_model, student_model, training_box.train_data_loader, device, 
        #                                          log_freq=log_freq, header="Train Agreement: ", train_agreement=True)
        #     logger.info('Training Top-1 agreement: {:.4f}'.format(train_top1_agreement))
...
def main(args):
    ...
    if (teacher_model and student_model):
        test_top1_agreement = get_agreement(teacher_model, student_model, test_data_loader, device,
                                            log_freq=log_freq, header="Test Agreement: ", train_agreement=False)
        logger.info('Test Top-1 agreement: {:.4f}'.format(test_top1_agreement))
